    ...
    steps {
         script {
                if (env.GIT_BRANCH == 'develop') {
                    ...
                    sh "./gradlew -b "+buildFilePath+" sonar -Dsonar.host.url=${SONAR_URL} -Dsonar.login=${SONAR_PASSWORD} -Dsonar.github.repository=${env.CUSTOM_SONAR_REPO_NAME} -Dsonar.branch=${env.GIT_BRANCH} -i"
                }
         }
    }
   }
