        ...
        step {
            script {
                if (env.GIT_BRANCH == 'develop') {
                    ...
                    sh "./gradlew -b roostify-product-pricing/build.gradle sonar -Dsonar.github.pullRequest=${env.CHANGE_ID} -Dsonar.host.url=${SONAR_URL} -Dsonar.login=${SONAR_PASSWORD} -Dsonar.analysis.mode=preview -Dsonar.github.oauth=${GITHUB_OAUTH_TOKEN} -Dsonar.github.repository=${env.CUSTOM_SONAR_REPO_NAME} -i"
                }else{
